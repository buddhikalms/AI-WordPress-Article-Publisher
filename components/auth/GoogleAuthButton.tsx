...
export default function GoogleAuthButton({callbackUrl="/app/dashboard"}:{callbackUrl?:string}){const[loading,setLoading]=useState(false);return <Button type="button" variant="outline" className="w-full" disabled={loading} onClick={async()=>{setLoading(true);await signIn("google",{callbackUrl});}}>{loading?<Loader2 className="h-4 w-4 animate-spin"/>:<GoogleIcon/>}{loading?"Connecting...":"Continue with Google"}</Button>}
